...
const signUpUser = async (req: express.Request, res: express.Response) => {
  ...
  try {
    ...
    if (check_user_exist) {
      return res
        .status(404)
        .send({ status: 404, message: "User already exists!" });
    }
...
const loginUser = async (req: express.Request, res: express.Response) => {
  ...
  try {
    ...
    if (user) {
      ...
      res
        .status(200)
        .send({ status: 200, message: user._id, role: user.role, jwt: token });
    } else {
      return res
        .status(404)
...
const getCurrentUser = async (req: express.Request, res: express.Response) => {
  try {
    ...
    const id = req.headers.authorization
      ? userMiddleware.getUserIdFromToken(
    ...
    if (!userId) {
      return res
        .status(401)
        .send({ status: 401, message: "User not authenticated" });
    }
