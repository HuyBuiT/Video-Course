...
export const getPartByNameAndCourseId = async (
  name: string,
  courseId: string
) => {
  try {
    ...
    if (error instanceof Error) {
      throw new Error(
        `Could not get part by name and courseId: ${error.message}`
      );
    } else {
...
export const createOrUpdatePart = async (data: {
  name: string;
  courseId: string;
  path: string;
}) => {
  try {
    ...
    if (!existingPart) {
      ...
      const updatedPart = await updatePart(existingPart._id, {
        name,
        courseId,
        path,
      }); // Call the updatePart function
      return updatedPart;
    }
  } catch (error) {
    if (error instanceof Error) {
