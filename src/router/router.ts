...
import * as lessonModel from "../db/lesson";
import fs from "fs";
...
router.get("/parts/:id", middlewareUser.verifyToken, scan.getParts);
router.get("/lessons/:id", middlewareUser.verifyToken, scan.getLessons);
router.get("/courses", middlewareUser.verifyToken, scan.getCourses);
router.get("/video/:id", middlewareUser.verifyToken, scan.getVideo);
router.get(
  "/play_video",
  async (req: express.Request, res: express.Response) => {
    ...
    try {
      const response = await lessonModel.getLessonsByLessonId(videoId);
      const videoPath = response?.file;
      if (videoPath) {
        ...
        console.error("Video Path undefined");
        res.status(500).send("Video Path undefined");
      }
    } catch (error) {
